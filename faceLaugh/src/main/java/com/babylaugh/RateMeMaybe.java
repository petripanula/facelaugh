...
import com.babylaugh.RateMeMaybeFragment.RMMFragInterface;

public class RateMeMaybe implements RMMFragInterface {
	private static final String TAG = "Pete";
	...
	public static void resetData(FragmentActivity activity) {
		...
		if(BabyMain.ENABLE_LOGS) Log.v(TAG, "Cleared RateMeMaybe shared preferences.");
	}
	...
	public void run() {
		...
		if (!isPlayStoreInstalled()) {
			if(BabyMain.ENABLE_LOGS) Log.v(TAG, "No Play Store installed on device.");
	...
	public void _callWhenRatingStarted(){
		if (mListener != null) {
			mListener.RatingStarted();
		}
	}
